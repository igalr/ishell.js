...
export const handlers = {
  matchHandler(input: any): InputHandler {
    if (LambdaURLHandler.isLambdaURL(input)) return new LambdaURLHandler(input as Record<string, any>);
    if (ExpressURLHandler.isExpressURL(input)) return new ExpressURLHandler(input as Record<string, any>);
    if (AmazonConnectHandler.isAmazonConnect(input)) return new AmazonConnectHandler(input as Record<string, any>);
